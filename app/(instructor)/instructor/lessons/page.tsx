  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto py-4 px-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b pb-4">
        <div>
          <Link href={`/instructor/course-detail?courseId=${courseId}`} className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-800 transition mb-2">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100">
            {lessons.length > 0 ? (
                ...
                return (
                  <tr key={lesson._id} className="hover:bg-slate-50/50 transition text-sm">
                    ...
                    <td className="p-4">
                      {matchingQuiz ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-500">({matchingQuiz.questions?.length || 0} câu)</span>
                        </div>
                      ) : (
                    ...
                    <td className="p-4 text-right space-x-1 whitespace-nowrap">
                      {/* TÁC VỤ QUIZ */}
                      {!matchingQuiz ? (
                        <Link href={`/instructor/quiz-create?courseId=${courseId}&lessonId=${lesson._id}`} className="inline-flex text-blue-600 hover:bg-blue-50 px-2.5 py-1.5 border border-blue-200 rounded-lg text-xs font-bold">
                          + Quiz
                        </Link>
                      ) : (
                        <>
                          ...
                          <Link href={`/instructor/quiz-edit?courseId=${courseId}&lessonId=${lesson._id}`} className="inline-flex text-amber-600 hover:bg-amber-50 px-2.5 py-1.5 border border-amber-200 rounded-lg text-xs font-bold">
                            Sửa Quiz
                          </Link>
                      ...
                      {/* TÁC VỤ LESSON */}
                      <Link href={`/instructor/lesson-detail?courseId=${courseId}&lessonId=${lesson._id}`} className="inline-flex text-slate-700 hover:bg-slate-100 px-2.5 py-1.5 border border-slate-300 rounded-lg text-xs font-bold">
                        Sửa Bài
                      </Link>
              ...
              <tr>
                <td colSpan={4} className="p-12 text-center text-slate-500 text-xs">📭 Chưa có bài giảng nào trong hệ thống cấu trúc nháp này.</td>
              </tr>
            )}
          </tbody>
        </table>
