  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6 text-slate-700">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-xs text-slate-500 font-medium">Tỷ lệ Đạt (Pass)</p>
          <p className="text-2xl font-bold text-emerald-600 mt-1">{stats?.passRate || 0}%</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-xs text-slate-500 font-medium">Chưa hoàn thành</p>
      ...
      <div className="flex border-b border-slate-200 gap-6 text-sm font-bold">
        <button
          ...
          className={`pb-3 relative transition-colors ${activeTab === "submitted" ? "text-blue-600 border-b-2 border-blue-600" : "text-slate-500 hover:text-slate-600"}`}
        >
          Đã làm bài ({stats?.submittedList?.length || 0})
        </button>
        <button
          onClick={() => setActiveTab("unsubmitted")}
          className={`pb-3 relative transition-colors ${activeTab === "unsubmitted" ? "text-amber-600 border-b-2 border-amber-600" : "text-slate-500 hover:text-slate-600"}`}
        >
          ...
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {stats?.submittedList?.map((item: any) => (
                <tr key={item._id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    ...
                    <span className="text-xs text-slate-500 block">Tỷ lệ chính xác: {item.percentage}%</span>
                  </td>
                  <td className="p-4">
                    {item.passed ? (
                  ...
                  <td className="p-4 text-slate-500 text-xs">
                    ...
                    <p className="text-[10px] text-slate-500">Lượt làm: Thứ #{item.attemptNumber}</p>
                  </td>
              ...
              {stats?.submittedList?.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center py-10 text-slate-500 text-xs italic">Chưa có học sinh nào nộp bài tập này.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full border border-slate-200 shadow-xl space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Học sinh được chọn: <span className="font-semibold text-slate-700">{selectedStudent.name}</span></p>
              </div>
            </div>
