              ...
              <div className="mt-6 space-y-3 border-t border-slate-100 pt-5 text-sm">
                {activity && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <span className="text-slate-700">
                      <strong className="text-slate-900">{activity.currentStreak}</strong> ngày học liên tiếp
                      <span className="mx-1.5 text-slate-400">·</span>
                      Dài nhất: <strong className="text-slate-900">{activity.longestStreak}</strong> ngày
                    </span>
                  </div>
                )}
                ...
                {activity && (
                  <div className="flex items-start gap-2.5">
                    ...
                    <span className="text-slate-700">
                      <strong className="text-slate-900">{activity.activeDays}</strong> ngày có hoạt động
                      <span className="mx-1.5 text-slate-400">·</span>
                      <strong className="text-slate-900">{activity.total}</strong> hoạt động
                    </span>
                  </div>
                )}
          ...
          <section className="space-y-6 lg:col-span-8 xl:col-span-9">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Họ và tên đầy đủ" value={user.fullname || "Chưa cập nhật"} />
...
function describeJoined(v?: string) {
  if (!v) return "";
  const months = Math.floor((Date.now() - new Date(v).getTime()) / (1000 * 60 * 60 * 24 * 30));
