            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm pt-2 text-slate-700">
              {stats && (
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-slate-400">({stats.totalReviews} đánh giá)</span>
                </div>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
            ...
            <section id="curriculum" className="space-y-6 scroll-mt-20">
              ...
              <div className="border border-slate-200 rounded-xl divide-y divide-slate-100 overflow-hidden shadow-sm">
                {course.lessons && course.lessons.length > 0 ? (
                    ...
                    .map((lesson: any, index: number) => (
                      ...
                      >
                        <div className="flex items-center gap-4 min-w-0">
                          <span className="w-6 text-sm font-bold text-slate-400 text-center group-hover:text-blue-600">
                            {index + 1}
                          </span>
                          <CheckCircle size={16} className="text-slate-300 group-hover:text-blue-600 flex-shrink-0 transition-colors" />
                  ...
                  <p className="text-sm text-slate-400 italic py-8 text-center bg-slate-50/50">Nội dung bài học hiện tại đang được xây dựng.</p>
                )}
              </div>
            </section>
            ...
            <section id="reviews" className="space-y-6 scroll-mt-20">
              ...
              {isEnrolled && userProgress >= 25 ? (
                ...
                <div className="p-4 border border-dashed border-slate-200 rounded-xl bg-slate-50 text-center">
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">Tiến trình lớp học hiện tại của bạn: {userProgress}%</p>
                </div>
              ) : null}
              ...
              <div className="space-y-4">
                {loadingReviews ? (
                  <p className="text-xs text-slate-400 text-center py-4">Đang đồng bộ bình luận...</p>
                ) : reviews.length > 0 ? (
                  reviews.map((review) => (
                    <div key={review._id} className="p-5 border border-slate-200 rounded-xl bg-white space-y-3 shadow-sm">
                      ...
                      <div className="flex items-center gap-4 pt-1 pl-1">
                        <button 
                          ...
                          className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-blue-700 font-semibold transition"
                        >
                          <ThumbsUp size={13} />
                  ...
                  <p className="text-sm text-slate-400 italic py-6 text-center border border-dashed rounded-xl">Khóa học này hiện chưa nhận được phản hồi.</p>
                )}
              </div>
            </section>
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-24 space-y-4">
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-md space-y-4">
              <div className="space-y-1">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Mức giá chương trình</span>
