...
import {
  Plus, Search, Edit3, Trash2, Lock, Unlock, X, Loader2,
  ShieldCheck, GraduationCap, User as UserIcon, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const ROLE_STYLE: Record<string, { cls: string; Icon: any }> = {
...
const emptyForm = {
  ...
  role: "student" as AdminUser["role"], status: true,
};
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setMyId(JSON.parse(raw)._id ?? null);
    } catch {
      /* bo qua */
    }
  }, []);
  ...
  const toggleStatus = async (u: AdminUser) => {
    try {
      setBusyId(u._id);
      setError("");
      await updateUserStatusAdmin(u._id, !u.status);
      await load();
    } catch (e: any) {
                    ...
                    <tr key={u._id} className="transition hover:bg-slate-50/60">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-sm font-bold uppercase text-white">
                            {u.name?.charAt(0) || "?"}
                          </div>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 rounded-lg border px-2 py-1 text-xs font-semibold capitalize ${cls}`}>
                          <Icon size={12} /> {u.role}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 text-xs font-semibold ${u.status ? "text-emerald-700" : "text-red-700"}`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${u.status ? "bg-emerald-500" : "bg-red-500"}`} />
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            title={isSelf ? "Không thể tự khóa chính mình" : u.status ? "Khóa" : "Mở khóa"}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <form onSubmit={submit} className="space-y-4 px-5 py-5">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-bold text-slate-700">Tên *</label>
                <input
                  value={form.name}
              ...
              <div>
                ...
                <input
                  ...
                  className={inputCls}
                  placeholder={editingId ? "Không đổi" : "Tối thiểu 6 ký tự"}
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1.5 block text-xs font-bold text-slate-700">Quyền</label>
                  <select
                    value={form.role}
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-bold text-slate-700">Trạng thái</label>
                  <select
