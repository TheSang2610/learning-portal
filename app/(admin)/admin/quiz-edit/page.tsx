...
import { getCourseQuizzes, updateQuiz } from "@/src/services/quizService"; // Đổi sang getCourseQuizzes

function AdminEditQuizPageContent() {
  const params = useSearchParams();
  const router = useRouter();
  
  // 🔎 Lấy đúng tên thư mục động trong cấu trúc của bạn
  const courseId = params.get("courseId") || "";
  const lessonId = params.get("lessonId") || ""; 

  const [targetQuizId, setTargetQuizId] = useState<string>(""); // Lưu id thực tế của Quiz sau khi tìm thấy
  ...
  // Đọc dữ liệu Quiz thông qua courseId và lessonId
  useEffect(() => {
    const fetchQuizDetails = async () => {
      try {
        setLoading(true);
        // Gọi API lấy quiz của khóa học, lọc theo đúng bài học hiện tại
        const dataList = await getCourseQuizzes(courseId, lessonId); 
        
        // Tìm bài kiểm tra khớp với lessonId này
        ...
        if (currentQuiz) {
          setTargetQuizId(currentQuiz._id); // Lưu lại Id để tí nữa gửi lệnh PUT update
          setQuizConfig({
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateQuiz(targetQuizId, payload); // Sử dụng id thật đã lưu trong State
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 space-y-6">
      <div>
        <Link href={`/admin/lessons?courseId=${courseId}`} className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-slate-800 transition">
        ...
        <div className="space-y-4">
          ...
          {questions.map((question, qIndex) => (
            <div key={qIndex} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm relative">
              <button type="button" onClick={() => removeQuestion(qIndex)} className="absolute top-4 right-4 text-slate-500 hover:text-red-500 transition"><Trash2 size={16} /></button>
